...
Status: **Phases 1–9 implemented** (Go backend, REST API, JWT auth, SQLite, HTTPS + multi-provider CT scanners, Bootstrap 5 web UI, cron notifications, inventory reports, backup/restore scripts, bulk import, groups, tags, domain update, OpenAPI docs). All-valid-certificate discovery with normalized-DN dedup (CertSpotter + ctlogs.dev). Security audit completed — **28/28 issues fixed**.
